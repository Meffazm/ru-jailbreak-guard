#!/bin/bash
# Phase 8 — bootstrap a freshly-applied YC k8s cluster with ArgoCD + ApplicationSet.
#
# Run AFTER `cd infra && terraform apply` succeeds.
#
#   ./scripts/bootstrap_cloud.sh
#
# Prereqs: yc CLI configured (yc init / yc config set), kubectl installed.
...
echo "[4/5] Applying project + ApplicationSet (points at main, picks up gitops/apps/*)..."
...
echo ""
echo "  Capture defense screenshots per docs/runbooks/cloud-defense-capture.md"
echo ""
echo "  TEAR DOWN AFTER CAPTURE TO PRESERVE BUDGET:"
echo "    ./scripts/teardown_cloud.sh"
