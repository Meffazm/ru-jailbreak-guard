...
@production alias, opens a PR with updates.

The values.yaml key is `modelVersion` (lower-camel); the predictor pods see
this rendered into the `MODEL_VERSION_PIN` env var via Helm template (see
`gitops/apps/model-*/templates/isvc.yaml`). This script bumps the source
of truth — the values.yaml — and ArgoCD propagates it to the ISVC.

No-ops cleanly when MLFLOW_TRACKING_URI is unset (local-first scope; cloud
install at Phase 8 sets it).
"""
